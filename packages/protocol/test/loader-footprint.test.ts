 ...
 * site, and its CI holds it to 6 KB gzip. That budget is enforced at the
 * workspace root by `scripts/check-budgets.mjs`, against the built bundle,
 * which means a regression introduced here is discovered one package away from
 * its cause. This measures the same number at the source.
 *
...
describe('loader footprint', () => {
  it('stays small enough to be worth sharing', async () => {
    ...
    // Generous against today's ~960 B, which moves a few bytes with the esbuild
    // and zlib versions in play — this is a regression alarm, not a target.
    // Raise it in a PR that says what got bigger and why.
