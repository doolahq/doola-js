...
| `mint_failed`             | First token fetch failed for a transient or unknown reason (network, 5xx from their route, malformed response). Statuses with their own meaning are mapped to the cases above, never here. | Retry / surface their own error state.                                                                 |
