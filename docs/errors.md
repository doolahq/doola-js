...
| `mint_failed`             | First token fetch failed for a transient or unknown reason: network, malformed response, or a 5xx from their route — which includes doola-side auth/config failures the route surfaces as 502 (a broken `dk_` key is the partner's problem, never the customer's session). Statuses with their own meaning are mapped to the cases above, never here. | Retry / surface their own error state.                                                                 |
